...
라이브러리 설치 등 로컬 환경 설정 문제는 발생하지 않습니다.
단, Azure OpenAI API 키/엔드포인트 설정 및 API 호출 관련 오류는 발생할 수 있습니다.
...
- **PLATFORM_API_USAGE**: Azure OpenAI API 사용법 및 에러 (인증, 호출 방법, 응답 파싱 등)
...
2. **11개 카테고리에 맞지 않음**
3. **여러 주제 혼재**
4. **문의 내용 불명확** (예: "과제가 안돼요", "오류가 계속 납니다")
...
- 매우 명확한 키워드 (예: "최종 과제 재제출 가능 여부", "pip install 패키지 설치 가능 여부")
- 구체적이고 명확한 문의
- 하나의 카테고리에 완벽히 매칭
- 과거 사례가 많을 것으로 예상
