...
> |--------|----------|----------|
> | `main` | label-aware | 없음 |
> | `indiv` | label-aware | user_db.py (수강생 SQLite) |
> | `indiv_nolabel` | similarity-only (label 무시) | user_db.py (수강생 SQLite) |
...
├── user_db.py                    # 수강생 개인화 DB (SQLite) — indiv/indiv_nolabel 브랜치
...
[Step 3] RAG 검색 (tool_rag / human_review)
  ① FAISS 벡터 검색 (Azure text-embedding-3-large, 3072차원)
     - indiv 브랜치:         label-aware (동일 label 우선, cosine 유사도 top-3)
     - indiv_nolabel 브랜치: similarity-only (label 무시, 순수 유사도 top-3)
