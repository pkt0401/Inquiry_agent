...
├── info/
│   ├── kb/
│   │   └── knowledge_base.json   # 라벨별 큐레이션 Q&A + 사전 지식 (현재 v2.0.0, 2026-04-16 갱신)
│   ├── schedule/                 # 프로그램별 일정 (기수 변경 시 해당 파일만 갈아끼움)
│   │   ├── bootcamp.json         # AI Bootcamp 일정 (현재 14기(8차), 9차~14차 예정)
│   │   ├── literacy.json         # AI Literacy 인증시험 일정 (현재 14차)
│   │   └── master_project.json   # AI Master Project 일정 (현재 5기 진행중)
│   ├── past/                     # 과거 버전 보관 (파일명에 유효 종료일 표기)
│   │   ├── knowledge_base_v1_~20260403.json
│   │   ├── schedule_v12_~20260320.json
│   │   └── schedule_v13_~20260515.json
│   ├── inquiry_all.json          # 전체 문의 게시물 (315건)
│   ├── inquiry_comment_all.json  # 전체 문의 댓글 (344건, admin 289건)
│   └── files.json                # 첨부파일 메타데이터
│
├── files/                        # 실제 첨부파일
├── tests/                        # 테스트셋 JSON 및 평가 결과
├── README.md                     # 본 파일
└── PoC_Report.md                 # PoC 상세 결과 (흐름 설계 + 분석)
```

> 운영자 ID: 2, 7, 61, 442, 2425, 3417 (구 형식) + `is_admin=1` 플래그 (신 형식, 2026-03 이후)
...
| 구분 | 파일 | 문의 | 댓글 | 비고 |
|------|------|-----:|-----:|------|
| 전체 (train+test) | inquiry_all.json + inquiry_comment_all.json | 315건 | 344건 | ~2026-04-14 기준, admin 답변 289건 (274 inquiry) |
| 테스트 (평가용) | inquiry_all.json 내 random 샘플링 또는 외부 test file | 가변 | — | `--n-test`/`--test-file`로 조절, leakage 방지 |
| RAG 풀 (학습용) | inquiry_all.json 내 나머지 (admin 답변 보유) | ~270건 | — | 테스트 제외 후 인덱싱 |

> 관리자 답변 필터: `author_id in ADMIN_IDS` OR `is_admin=1` (신 데이터 포맷 호환)
...
# 배치 평가 모드 (기본)
python inquiry_agent.py                                   # inquiry_all.json에서 random 샘플링
python inquiry_agent.py --n-test 20 --random-state 12     # 샘플 수·시드 지정
python inquiry_agent.py --test-file tests/test_set.json   # 외부 테스트셋 평가

# 대화형 모드 (REPL)
python inquiry_agent.py --interactive --author-id 277     # 고정 author_id로 문의 입력
```
...
[Step 1] LLM 분류  →  label(12개) + confidence_level(high/low) + is_compound + sub_labels
          ↑ prior_knowledge + 라벨별 설명·예시 (info/kb/knowledge_base.json) 주입
          ↑ 기수 일정·인증시험 일정 (info/schedule/ 프로그램별 파일 자동 합성) 주입
